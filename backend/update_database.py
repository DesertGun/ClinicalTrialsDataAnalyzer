...
def get_new_chunk(current_border, tmp_border):
    ...
    url_new_chunk = "https://clinicaltrials.gov/api/query/study_fields?min_rnk=" + current_border + "&max_rnk="+tmp_border + \
        "&fields=NCTId,Condition,OverallStatus,PrimaryOutcomeMeasure,PrimaryOutcomeTimeFrame,SecondaryOutcomeMeasure,SecondaryOutcomeTimeFrame,WhyStopped,BaselineMeasureTitle&fmt=csv"
    try:
...
def download_data():
    ...
    while tmp_border < max_border:
        ...
        input_data_current_chunk = get_new_chunk(
            str(current_border), str(tmp_border))
    ...
    input_data_last_chunk = get_new_chunk(
        str(current_border), str(current_border + diff_border))
